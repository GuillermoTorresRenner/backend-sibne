...
---
## ✅ Cambios aplicados al modelo de base de datos

### 1) Tablas eliminadas
Se eliminaron por obsolescencia y reorganización del modelo:
- `CargaMasivaArchivo`
- `CargaMasivaDetalle`
- `CargaMasivaError`
- `Usuario`
...
- Normalización de relaciones entre `Comuna`, `Provincia` y `Region`.
- Se agrega campo `password` a `Contacto`, con nuevas relaciones hacia `Role` y `EmailLog`.
- `UsuarioLogin` pasa a llamarse `ContactoLogin` y se asocia a `Contacto` y al enum `LoginProviders`.
- `EmailConfig` deja de relacionarse con `EmailLogs` y pasa a vincularse con `ListaEmpresasEmail`.
- `EmailLogs`:
  - Eliminación de relación directa con `Empresa`.
  - Nueva relación con `Contacto` y `EstadoEmail`.
  - Eliminación de campos: `fechaHoraRegistro`, `para`, `msje`, `empresaId`.
  - Simplificación temporal: se mantiene únicamente `fechaHoraEnvio`.

### 3) Nuevas estructuras creadas
- Enum `LoginProviders` para estandarizar métodos de autenticación.
- Tabla `ListaEmpresasEmail` para administrar la vinculación entre empresas y configuraciones de email.
